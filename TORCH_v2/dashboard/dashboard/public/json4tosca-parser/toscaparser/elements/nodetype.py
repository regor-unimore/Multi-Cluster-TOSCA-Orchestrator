...
class NodeType(StatefulEntityType):
    ...
    def interfaces(self):
        ifaces = {}
        iface = None
        if self.ntype in self.custom_def:
           iface = self.get_value(self.INTERFACES, defs=self.custom_def[self.ntype])
        else:
           iface = self.get_value(self.INTERFACES)
        if iface:
           ifaces.update(iface)
        return ifaces
    ...
    def artifacts(self):
        ...
        a = None
        if self.ntype in self.custom_def:
           a = self.get_value(self.ARTIFACTS, defs=self.custom_def[self.ntype])
        else:
           a = self.get_value(self.ARTIFACTS)
        if a:
           arts.update(a)
        #art2 = self.get_definition(self.ARTIFACTS)
        #print("arts from artifacts property in NodeType from get_definition is " + str(art2))
        return arts
