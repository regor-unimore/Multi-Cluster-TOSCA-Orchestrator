...
class GetInput(Function):
    ...
    * get_input: port
    ...
    def result(self):
        ...
        return "input", self.input_name, input.default 
    ...
    * { get_attribute: [ HOST, private_address, 0, some_prop] }
    ...
    def validate(self):
        if len(self.args) < 2:
            ...
            if node_tpl is None:
                return
            self.target_node = node_tpl.name
            index = 2
    ...
    def result(self): 
        result = []
        if self.target_type and self.target_type == "capability":
           result = [self.target_node, self.target_capability, self.target_attribute]
        else:
           result = [self.target_node, self.target_attribute]
        ...
        for r in node_tpl.requirements:
            for req, node_name in r.items():
                if req == req_or_cap:
                    ...
                    # Find capability linked to this requirement
                    capability = self._find_linked_capability(node_tpl, node_template, req)
                    if not capability:
                        capability = req  #back to how it was implemented before
                    self.target_node = node_template.name
                    self.target_type = "capability"
    ...
    def _find_linked_capability(self, node_tpl, target_node_tpl, requirement):
        capability = None
        for req in node_tpl.type_definition.requirements:
            for key, value in req.items():
                if key == requirement:
                    target_capability_type = value["capability"]
                    #target_rel_type = value["relationship"]
                    for c in target_node_tpl.type_definition.get_capabilities_objects():
                        hierarchy = [c.type]
                        t = c.parent_type
                        while t:
                             hierarchy.append(t.type)
                             t = t.parent_type
                        if target_capability_type in hierarchy:
                            capability = c.name
                            break    
                    break
        return capability
    ...
    * { get_property: [ SELF, database_endpoint, port, 1 ] }
    ...
    def result(self):
        if len(self.args) >= 3:
            ...
            if found:
                property_value = found[0].value
                property_name = found[0].name
            else:
                index = 3
                # then check the req or caps
                property_value = self._find_req_or_cap_property(self.args[1],
                                                                self.args[2])
                property_name = self.args[1] + ":" +  self.args[2]                                         
            if len(self.args) > index:
                for elem in self.args[index:]:
                    if isinstance(property_value, list):
                        int_elem = int(elem)
            ...
            property_name = self.args[1].name
        if isinstance(property_value, Function):
            return "property", property_name, property_value.result()
        return "property", property_name, get_function(self.tosca_tpl,
                            self.context,
                            property_value)
